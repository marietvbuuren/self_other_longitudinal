function soconnect_lt_twoflex_group_anova
%% filler script for mixed flexible factorial anova whole-brain analyses
...
dirs.outputroot = fullfile(dirs.home,'MRI','Experimental','data_group','MT','whole_brain','w3_students');
...
for i=1:numel(subjects_adolescents)
    ...
    funcfiles=[];
    for c=5:7,
        funcfiles= [funcfiles;spm_select('FPList',[dirs.input,'/'],['SoConnect_3_',subjname,'_con_000',num2str(c),'.nii'])];
    end
...
for j=1:numel(subjects_adults)
    ...
    funcfiles=[];
    for c=5:7,
        funcfiles= [funcfiles;spm_select('FPList',[dirs.input,'/'],['SoConnect_9_',subjname,'_con_000',num2str(c),'.nii'])];
    end
    k=j+add;
    matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(k).scans=cellstr(funcfiles);
end

jobfilename = ['twoway_flex_group.mat'];
