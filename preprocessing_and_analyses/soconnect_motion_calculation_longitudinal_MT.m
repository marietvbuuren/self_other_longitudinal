function soconnect_motion_calculation_longitudinal_MT

%% to calculate scan to scan motion, or framewise displacement (FD) & absolute motion 
...
if info.wave==1,
    subjects = [1,2,3,4,6:1:20,22:1:51,53:1:86];%%all subjects
elseif info.wave==2,
...
for isubject = 1:numel(subjects)
 ...
 if subj<10,
     niidirsubj= fullfile(dirs.root,'Experimental', 'data_indiv',['w',wave],['SoConnect_',wave,'_0',num2str(subj)],'MT');
     subjname = ['SoConnect_',wave,'_0',num2str(subj)];
else
    niidirsubj= fullfile(dirs.root,'Experimental', 'data_indiv',['w',wave],['SoConnect_',wave,'_',num2str(subj)], 'MT');
...
fid = fopen(fullfile(dirs.output,['FD_MT_w_',wave,'.txt']),'w+');
